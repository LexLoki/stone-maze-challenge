...
stack<vector<unsigned short>> *solve(unsigned length, unsigned short row, unsigned short col)
{
    ...
    if(visited[length][pos]) return nullptr;    // Already visited this coordinate at this time
    if(automata[length][pos]) return nullptr;   // Cell is live, forbidden
    if(row+col >= max_row+max_col) // Store "longer" paths
    {
        max_row = row; max_col = col;
        if(row+col > max_row+max_col || length < max_length) max_length = length;
    }
    if(row == trow && col == tcol){ // Objective reached
        //cout << "Sol found" << endl;
